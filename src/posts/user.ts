...
export default function (Posts: any) {
  Posts.getUserInfoForPosts = async function (uids: number[], uid: number) {
    ...
    const uidsSignatureSet = new Set<number>(
  ...
  async function checkGroupMembership(
    uid: number,
    groupTitleArray: string[] | undefined,
  ) {
  ...
  async function getUserData(uids: number[], uid: number) {
    ...
    return (await user.getUsersFields(
      result.uids,
      _.uniq(result.fields),
    )) as any[];
  }
  ...
  Posts.changeOwner = async function (pids: number | number[], toUid: number) {
    ...
    postData = postData.filter(
      (post: any) => post.pid && post.uid !== parseInt(toUid.toString(), 10),
    );
    ...
    await Promise.all([
      db.setObjectField(
        (pids as number[]).map((pid: number) => `post:${pid}`),
        "uid",
        toUid,
      ),
  ...
  async function handleMainPidOwnerChange(postData: any[], toUid: number) {
    ...
    const mainPosts = postData.filter(
      (post: any) => post.pid === tidToTopic[post.tid].mainPid,
    );
    if (!mainPosts.length) {
      return;
    }
    ...
    await Promise.all([
      db.setObjectField(
        mainPosts.map((p: any) => `topic:${p.tid}`),
        "uid",
        toUid,
      ),
      db.sortedSetRemoveBulk(bulkRemove),
